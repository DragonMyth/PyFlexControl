...
Vec3 SampleSDFGrad(const float* sdf, int dim, int x, int y, int z) {
	...
	float dx = (SampleSDF(sdf, dim, x1, y, z) - SampleSDF(sdf, dim, x0, y, z))
			* (dim * 0.5f);
	float dy = (SampleSDF(sdf, dim, x, y1, z) - SampleSDF(sdf, dim, x, y0, z))
			* (dim * 0.5f);
	float dz = (SampleSDF(sdf, dim, x, y, z1) - SampleSDF(sdf, dim, x, y, z0))
			* (dim * 0.5f);
...
void CreateGranularGrid(Vec3 center, Vec2 dimx, Vec2 dimz, float radius,
	...
	float gapx = (dimx.y - dimx.x) / particleDims.x;
	float gapz = (dimz.y - dimz.x) / particleDims.y;

	for (int x = 0; x < particleDims.x; ++x) {
		for (int z = 0; z < particleDims.y; ++z) {

			float posX = dimx.x + x * gapx;
			float posZ = dimz.x + z * gapz;
			if (rigid)
...
void CreateGooGrid(Vec3 center, Vec2 dimx, Vec2 dimy, Vec2 dimz, float radius,
		Vec3 particleDims, Vec3 velocity, float invMass, bool rigid,
		float rigidStiffness, int phase, float jitter = 0.005f) {
	if (rigid && g_buffers->rigidIndices.empty())
		g_buffers->rigidOffsets.push_back(0);

	float gapx = (dimx.y - dimx.x) / particleDims.x;
	float gapy = (dimy.y - dimy.x) / particleDims.y;

	float gapz = (dimz.y - dimz.x) / particleDims.z;

	for (int x = 0; x < particleDims.x; ++x) {
		for (int y = 0; y < particleDims.y; y++) {
			for (int z = 0; z < particleDims.z; ++z) {

				float posX = dimx.x + x * gapx;
				float posY = dimy.x + y * gapy;
				...
				Vec3 position = center + Vec3(posX, posY, posZ)
						+ RandomUnitVector() * jitter;
...
void CreateParticleSphere(Vec3 center, int dim, float radius, Vec3 velocity,
		float invMass, bool rigid, float rigidStiffness, int phase,
		float jitter = 0.005f) {
	if (rigid && g_buffers->rigidIndices.empty())
		g_buffers->rigidOffsets.push_back(0);
		...
		for (int y = 0; y <= dim; ++y) {
			for (int z = 0; z <= dim; ++z) {
				...
				if (sx * sx + sy * sy + sz * sz <= float(dim * dim) * 0.25f) {
					...
					Vec3 position = center + radius * Vec3(sx, sy, sz)
							+ RandomUnitVector() * jitter;
...
void CreateSpring(int i, int j, float stiffness, float give = 0.0f) {
	...
	g_buffers->springLengths.push_back(
			(1.0f + give)
					* Length(
							Vec3(g_buffers->positions[i])
									- Vec3(g_buffers->positions[j])));
	g_buffers->springStiffness.push_back(stiffness);
}

void CreateParticleShape(const Mesh* srcMesh, Vec3 lower, Vec3 scale,
		float rotation, float spacing, Vec3 velocity, float invMass, bool rigid,
		float rigidStiffness, int phase, bool skin, float jitter = 0.005f,
		Vec3 skinOffset = 0.0f, float skinExpand = 0.0f,
		Vec4 color = Vec4(0.0f), float springStiffness = 0.0f) {
	if (rigid && g_buffers->rigidIndices.empty())
		g_buffers->rigidOffsets.push_back(0);
...

	{
		...
		Matrix44 xform = ScaleMatrix(scale / maxEdge)
				* TranslationMatrix(Point3(-meshLower));
		...
		Voxelize((const Vec3*) &mesh.m_positions[0], mesh.m_positions.size(),
				(const int*) &mesh.m_indices[0], mesh.m_indices.size(), maxDim,
				maxDim, maxDim, &voxels[0], meshLower,
			...
			for (int y = 0; y < maxDim; ++y) {
				for (int z = 0; z < maxDim; ++z) {
					...
					if (voxels[index]) {
						...
						Vec3 position = lower + meshLower
								+ spacing
										* Vec3(float(x) + 0.5f, float(y) + 0.5f,
												float(z) + 0.5f)
								+ RandomUnitVector() * jitter;

						// normalize the sdf value and transform to world scale
						Vec3 n = SafeNormalize(
								SampleSDFGrad(&sdf[0], maxDim, x, y, z));
		...
		mesh.Transform(
				ScaleMatrix(1.0f + skinExpand)
						* TranslationMatrix(
								Point3(-0.5f * (meshUpper + meshLower))));
		mesh.Transform(
				TranslationMatrix(
						Point3(lower + 0.5f * (meshUpper + meshLower))));
		...
		if (springStiffness > 0.0f) {
			...
			for (int x = 0; x < maxDim; ++x) {
				for (int y = 0; y < maxDim; ++y) {
					for (int z = 0; z < maxDim; ++z) {
						const int centerCell = z * maxDim * maxDim + y * maxDim
								+ x;
						...
						if (voxels[centerCell]) {
							...
							for (int i = x - width; i <= x + width; ++i) {
								for (int j = y - width; j <= y + width; ++j) {
									for (int k = z - width; k <= z + width;
											++k) {
										const int neighborCell = k * maxDim
												* maxDim + j * maxDim + i;
										...
										if (neighborCell > 0
												&& neighborCell
														< int(voxels.size())
												&& voxels[neighborCell]
												&& neighborCell != centerCell) {
											CreateSpring(indices[neighborCell],
													indices[centerCell],
													springStiffness);
										}
									}
	...
	if (skin) {
		...
		const Colour colors[7] = { Colour(0.0f, 0.5f, 1.0f), Colour(0.797f,
				0.354f, 0.000f), Colour(0.000f, 0.349f, 0.173f), Colour(0.875f,
				0.782f, 0.051f), Colour(0.01f, 0.170f, 0.453f), Colour(0.673f,
				0.111f, 0.000f), Colour(0.612f, 0.194f, 0.394f) };

		for (uint32_t i = startVertex; i < g_mesh->GetNumVertices(); ++i) {
			int indices[g_numSkinWeights] = { -1, -1, -1, -1 };
			float distances[g_numSkinWeights] = { FLT_MAX, FLT_MAX, FLT_MAX,
					FLT_MAX };

			if (LengthSq(color) == 0.0f)
				g_mesh->m_colours[i] = 1.25f
						* colors[((unsigned int) (phase)) % 7];
			else
...
void CreateParticleShape(const char* filename, Vec3 lower, Vec3 scale,
		float rotation, float spacing, Vec3 velocity, float invMass, bool rigid,
		float rigidStiffness, int phase, bool skin, float jitter = 0.005f,
		Vec3 skinOffset = 0.0f, float skinExpand = 0.0f,
		Vec4 color = Vec4(0.0f), float springStiffness = 0.0f) {
	...
	if (mesh)
		CreateParticleShape(mesh, lower, scale, rotation, spacing, velocity,
				invMass, rigid, rigidStiffness, phase, skin, jitter, skinOffset,
				skinExpand, color, springStiffness);
			...
			for (int i = startVertex; i < numVertices + startVertex; ++i) {
				...
				for (int w = 0; w < 4; ++w) {
					...
					if (g_meshSkinIndices[i * 4 + w] > -1) {
						...
						skinPos +=
								(rotation
										* (g_meshRestPositions[i]
												- Point3(
														g_buffers->restPositions[index]))
										+ Vec3(g_buffers->positions[index]))
										* weight;
					}
				}
...
void CreateSDF(const Mesh* mesh, uint32_t dim, Vec3 lower, Vec3 upper,
	...
	if (mesh) {
		...
		Voxelize((const Vec3*) &mesh->m_positions[0], mesh->m_positions.size(),
				(const int*) &mesh->m_indices[0], mesh->m_indices.size(), dim,
				dim, dim, volume, lower, upper);
...
void AddRandomConvex(int numPlanes, Vec3 position, float minDist, float maxDist,
		Vec3 axis, float angle) {
	const int maxPlanes = 12;

	// 12-kdop
	const Vec3 directions[maxPlanes] = { Vec3(1.0f, 0.0f, 0.0f), Vec3(0.0f,
			1.0f, 0.0f), Vec3(0.0f, 0.0f, 1.0f), Vec3(-1.0f, 0.0f, 0.0f), Vec3(
			0.0f, -1.0f, 0.0f), Vec3(0.0f, 0.0f, -1.0f), Vec3(1.0f, 1.0f, 0.0f),
			Vec3(-1.0f, -1.0f, 0.0f), Vec3(1.0f, 0.0f, 1.0f), Vec3(-1.0f, 0.0f,
					-1.0f), Vec3(0.0f, 1.0f, 1.0f), Vec3(0.0f, -1.0f, -1.0f), };
	...
	g_buffers->shapePositions.push_back(
			Vec4(position.x, position.y, position.z, 0.0f));
	...
	NvFlexUpdateConvexMesh(g_flexLib, mesh, planes.buffer, planes.size(), lower,
			upper);
	...
	for (uint32_t j = 0; j < builder.mIndices.size(); j += 3) {
		...
		Vec3 n = Normalize(
				Cross(builder.mVertices[b] - builder.mVertices[a],
						builder.mVertices[c] - builder.mVertices[a]));
...
void CreateRandomBody(int numPlanes, Vec3 position, float minDist,
		float maxDist, Vec3 axis, float angle, float invMass, int phase,
		float stiffness) {
	// 12-kdop
	const Vec3 directions[] = { Vec3(1.0f, 0.0f, 0.0f), Vec3(0.0f, 1.0f, 0.0f),
			Vec3(0.0f, 0.0f, 1.0f), Vec3(-1.0f, 0.0f, 0.0f), Vec3(0.0f, -1.0f,
					0.0f), Vec3(0.0f, 0.0f, -1.0f), Vec3(1.0f, 1.0f, 0.0f),
			Vec3(-1.0f, -1.0f, 0.0f), Vec3(1.0f, 0.0f, 1.0f), Vec3(-1.0f, 0.0f,
					-1.0f), Vec3(0.0f, 1.0f, 1.0f), Vec3(0.0f, -1.0f, -1.0f), };
	...
	for (size_t v = 0; v < builder.mVertices.size(); ++v) {
		...
		for (size_t i = v + 1; i < builder.mVertices.size(); ++i) {
			...
			g_buffers->springLengths.push_back(
					Length(builder.mVertices[v] - builder.mVertices[i]));
			g_buffers->springStiffness.push_back(stiffness);
	...
	// lazy
	g_buffers->triangleNormals.resize(
			g_buffers->triangleNormals.size() + builder.mIndices.size() / 3,
			Vec3(0.0f));
}
...
NvFlexTriangleMeshId CreateTriangleMesh(Mesh* m) {
	...
	NvFlexUpdateTriangleMesh(g_flexLib, flexMesh, positions.buffer,
...
void AddSDF(NvFlexDistanceFieldId sdf, Vec3 translation, Quat rotation,
		float width) {
	NvFlexCollisionGeometry geo;
	geo.sdf.field = sdf;
	geo.sdf.scale = width;
	...
	g_buffers->shapeGeometry.push_back((NvFlexCollisionGeometry&) geo);
	g_buffers->shapeFlags.push_back(
			NvFlexMakeShapeFlags(eNvFlexShapeSDF, false));
}

inline int GridIndex(int x, int y, int dx) {
	return y * dx + x;
}

void CreateSpringGrid(Vec3 lower, int dx, int dy, int dz, float radius,
	...
	for (int z = 0; z < dz; ++z) {
		for (int y = 0; y < dy; ++y) {
			for (int x = 0; x < dx; ++x) {
				Vec3 position = lower
						+ radius * Vec3(float(x), float(z), float(y));
				...
				if (x > 0 && y > 0) {
					g_buffers->triangles.push_back(
							baseIndex + GridIndex(x - 1, y - 1, dx));
					g_buffers->triangles.push_back(
							baseIndex + GridIndex(x, y - 1, dx));
					g_buffers->triangles.push_back(
							baseIndex + GridIndex(x, y, dx));
					...
					g_buffers->triangles.push_back(
							baseIndex + GridIndex(x, y, dx));
					g_buffers->triangles.push_back(
							baseIndex + GridIndex(x - 1, y, dx));

					g_buffers->triangleNormals.push_back(
							Vec3(0.0f, 1.0f, 0.0f));
					g_buffers->triangleNormals.push_back(
							Vec3(0.0f, 1.0f, 0.0f));
				}
...
void CreateTetMesh(const char* filename, Vec3 lower, float scale,
	...
	if (f) {
			...
			if (fgets(line, 2048, f)) {
				...
				case 't': {
					if (firstTet) {
						...
						// normalize positions
						for (int i = vertOffset;
								i < int(g_buffers->positions.size()); ++i) {
							Vec3 p =
									lower
											+ (Vec3(g_buffers->positions[i])
													- meshLower) * scale
													/ maxEdge;
							g_buffers->positions[i] = Vec4(p,
									g_buffers->positions[i].w);
						}
					...
					int indices[4];
					sscanf(line, "t %d %d %d %d", &indices[0], &indices[1],
							&indices[2], &indices[3]);
		...
		for (TriMap::iterator iter = triCount.begin(); iter != triCount.end();
				++iter) {
...
int PickParticle(Vec3 origin, Vec3 dir, Vec4* particles, int* phases, int n,
...
void CalculateRigidCentersOfMass(const Vec4* restPositions,
		int numRestPositions, const int* offsets, Vec3* translations,
		const int* indices, int numRigids) {
...
void CalculateRigidLocalPositions(const Vec4* restPositions, const int* offsets,
		const Vec3* translations, const int* indices, int numRigids,
		Vec3* localPositions) {
	int count = 0;
...
void DrawImguiString(int x, int y, Vec3 color, int align, const char* s, ...) {
	...
	imguiDrawText(x, y, align, buf,
			imguiRGBA((unsigned char) (color.x * 255),
...
void DrawShadowedText(int x, int y, Vec3 color, int align, const char* s, ...) {
	...
	imguiDrawText(x + HELPERS_SHADOW_OFFSET, y - HELPERS_SHADOW_OFFSET, align,
			buf, imguiRGBA(0, 0, 0));
	imguiDrawText(x, y, align, buf,
			imguiRGBA((unsigned char) (color.x * 255),
					(unsigned char) (color.y * 255),
...
void DrawShadowedRect(float x, float y, float w, float h, Vec3 color) {
	imguiDrawRect(x + HELPERS_SHADOW_OFFSET, y - HELPERS_SHADOW_OFFSET, w, h,
			imguiRGBA(0, 0, 0));
	imguiDrawRect(x, y, w, h,
			imguiRGBA((unsigned char) (color.x * 255),
					(unsigned char) (color.y * 255),
...
void DrawLine(float x0, float y0, float x1, float y1, float r, Vec3 color) {
	imguiDrawLine(x0, y0, x1, y1, r,
			imguiRGBA((unsigned char) (color.x * 255),
					(unsigned char) (color.y * 255),
					(unsigned char) (color.z * 255)));
}

void DrawShadowedLine(float x0, float y0, float x1, float y1, float r,
		Vec3 color) {
	imguiDrawLine(x0 + HELPERS_SHADOW_OFFSET, y0 - HELPERS_SHADOW_OFFSET,
			x1 + HELPERS_SHADOW_OFFSET, y1 - HELPERS_SHADOW_OFFSET, r,
			imguiRGBA(0, 0, 0));
	imguiDrawLine(x0, y0, x1, y1, r,
			imguiRGBA((unsigned char) (color.x * 255),
					(unsigned char) (color.y * 255),
...
float CalculateRadius(const Vec3* particles, Vec3 center, const int* indices,
		int numIndices) {
	float radiusSq = 0.0f;
...
int CreateClusters(Vec3* particles, const float* priority, int numParticles,
		std::vector<int>& outClusterOffsets,
		std::vector<int>& outClusterIndices,
		std::vector<Vec3>& outClusterPositions, float radius, float smoothing =
				0.0f) {
	...
	while (seeds.size()) {
		...
		if (!used[seed.index]) {
			...
			for (int p = 0; p < numParticles; ++p) {
				float dSq = LengthSq(
						Vec3(particles[seed.index]) - Vec3(particles[p]));
				if (dSq <= radiusSq) {
...
int CreateLinks(const Vec3* particles, int numParticles,
		std::vector<int>& outSpringIndices,
		std::vector<float>& outSpringLengths,
		std::vector<float>& outSpringStiffness, float radius, float stiffness =
				1.0f) {
	const float radiusSq = sqr(radius);
...
void CreateSkinning(const Vec3* vertices, int numVertices, const Vec3* clusters,
		int numClusters, float* outWeights, int* outIndices, float falloff,
		float maxdist) {
...
void SampleMesh(Mesh* mesh, Vec3 lower, Vec3 scale, float rotation,
		float radius, float volumeSampling, float surfaceSampling,
	...
	if (volumeSampling > 0.0f) {
		...
		//Voxelize(*mesh, dx, dy, dz, &voxels[0], meshLower - Vec3(spacing*0.05f) , meshLower + Vec3(maxDim*spacing) + Vec3(spacing*0.05f));
		Voxelize((const Vec3*) &mesh->m_positions[0], mesh->m_positions.size(),
				(const int*) &mesh->m_indices[0], mesh->m_indices.size(),
				maxDim, maxDim, maxDim, &voxels[0], meshLower,
				meshLower + Vec3(maxDim * spacing));
	...
	mesh->Transform(
			ScaleMatrix(1.0f)
					* TranslationMatrix(
							Point3(-0.5f * (meshUpper + meshLower))));
	mesh->Transform(
			TranslationMatrix(Point3(lower + 0.5f * (meshUpper + meshLower))));
	...
	if (surfaceSampling > 0.0f) {
		...
		if (1) {
			for (int i = 0; i < 50000; ++i) {
				...
				Point3 pt = mesh->m_positions[a] * u + mesh->m_positions[b] * v
						+ mesh->m_positions[c] * w;
	...
	CreateClusters(&samples[0], &priority[0], samples.size(), clusterOffsets,
			clusterIndices, outPositions, radius);
...
void GetShapeBounds(Vec3& totalLower, Vec3& totalUpper) {
	...
	for (int i = 0; i < g_buffers->shapeFlags.size(); ++i) {
		...
		case eNvFlexShapeCapsule: {
			localLower = -Vec3(geo.capsule.halfHeight, 0.0f, 0.0f)
					- Vec3(geo.capsule.radius);
			localUpper = Vec3(geo.capsule.halfHeight, 0.0f, 0.0f)
					+ Vec3(geo.capsule.radius);
			break;
		}
		case eNvFlexShapeConvexMesh: {
			NvFlexGetConvexMeshBounds(g_flexLib, geo.convexMesh.mesh,
		...
		case eNvFlexShapeTriangleMesh: {
			NvFlexGetTriangleMeshBounds(g_flexLib, geo.triMesh.mesh, localLower,
					localUpper);
		...
		TransformBounds(localLower, localUpper,
				Vec3(g_buffers->shapePositions[i]),
				g_buffers->shapeRotations[i], 1.0f, worldLower, worldUpper);
