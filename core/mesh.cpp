...
void Mesh::CalculateNormals() {
	...
	for (int i = 0; i < numTris; ++i) {
		...
		Vec3 n = Cross(m_positions[b] - m_positions[a],
				m_positions[c] - m_positions[a]);
...
Mesh* ImportMeshFromPly(const char* path) {
	...
	for (uint32_t v = 0; v < numVertices; ++v) {
		...
		mesh->m_positions[v] = Point3(properties[0], properties[1],
				properties[2]);
	...
	for (uint32_t f = 0; f < numFaces; ++f) {
		...
		Vector3 n = SafeNormalize(Cross(v1 - v0, v2 - v0),
	...
	for (uint32_t i = 0; i < numVertices; ++i) {
		mesh->m_normals[i] = SafeNormalize(mesh->m_normals[i],
				Vector3(0.0f, 1.0f, 0.0f));
	}
...
Mesh* ImportMeshFromObj(const char* path) {
	...
	while (file) {
		...
		if (strcmp(buffer, "vn") == 0) {
			...
			if (faceIndexCount == 3) {
				...
				cout << "Face with more than 4 vertices are not supported"
						<< endl;
			}
	...
	for (uint32_t i = 0; i < m->m_normals.size(); ++i) {
		m->m_normals[i] = SafeNormalize(m->m_normals[i],
				Vector3(0.0f, 1.0f, 0.0f));
	}
...
Mesh* CreateCubeMesh() {
	const Point3 vertices[24] = { Point3(0.5, 0.5, 0.5), Point3(-0.5, 0.5, 0.5),
			Point3(0.5, -0.5, 0.5), Point3(-0.5, -0.5, 0.5), Point3(0.5, 0.5,
					-0.5), Point3(-0.5, 0.5, -0.5), Point3(0.5, -0.5, -0.5),
			Point3(-0.5, -0.5, -0.5), Point3(0.5, 0.5, 0.5), Point3(0.5, -0.5,
					0.5), Point3(0.5, 0.5, 0.5), Point3(0.5, 0.5, -0.5), Point3(
					-0.5, 0.5, 0.5), Point3(-0.5, 0.5, -0.5), Point3(0.5, -0.5,
					-0.5), Point3(0.5, 0.5, -0.5), Point3(-0.5, -0.5, -0.5),
			Point3(0.5, -0.5, -0.5), Point3(-0.5, -0.5, 0.5), Point3(0.5, -0.5,
					0.5), Point3(-0.5, -0.5, -0.5), Point3(-0.5, -0.5, 0.5),
	...
	const Vec3 normals[24] = { Vec3(0.0f, 0.0f, 1.0f), Vec3(0.0f, 0.0f, 1.0f),
			Vec3(0.0f, 0.0f, 1.0f), Vec3(0.0f, 0.0f, 1.0f), Vec3(1.0f, 0.0f,
					0.0f), Vec3(0.0f, 1.0f, 0.0f), Vec3(1.0f, 0.0f, 0.0f), Vec3(
					0.0f, 0.0f, -1.0f), Vec3(1.0f, 0.0f, 0.0f), Vec3(1.0f, 0.0f,
					0.0f), Vec3(0.0f, 1.0f, 0.0f), Vec3(0.0f, 1.0f, 0.0f), Vec3(
					0.0f, 1.0f, 0.0f), Vec3(0.0f, 0.0f, -1.0f), Vec3(0.0f, 0.0f,
					-1.0f), Vec3(-0.0f, -0.0f, -1.0f), Vec3(0.0f, -1.0f, 0.0f),
			Vec3(0.0f, -1.0f, 0.0f), Vec3(0.0f, -1.0f, 0.0f), Vec3(-0.0f, -1.0f,
					-0.0f), Vec3(-1.0f, 0.0f, 0.0f), Vec3(-1.0f, 0.0f, 0.0f),
			Vec3(-1.0f, 0.0f, 0.0f), Vec3(-1.0f, -0.0f, -0.0f) };

	const int indices[36] = { 0, 1, 2, 3, 2, 1, 8, 9, 4, 6, 4, 9, 10, 11, 12, 5,
			12, 11, 7, 13, 14, 15, 14, 13, 16, 17, 18, 19, 18, 17, 20, 21, 22,
			23, 22, 21 };
...
Mesh* CreatePanMesh(float radius_base, float radius_top, float depth,
	...
	m->m_positions[numVerts - 1] = Point3(0.0f);
//	m->m_positions[numVerts - 2] = Point3(0.0f);

	for (uint32_t i = 0; i < segments; ++i) {

		m->m_positions[i] = Point3(
				radius_base * Sin((float(i) / segments) * k2Pi), 0.0f,
				radius_base * Cos((float(i) / segments) * k2Pi));
...
//				radius_base * Cos((float(i) / segments) * k2Pi));
//
//		m->m_positions[segments*3 + i] = Point3(
//				radius_top * Sin((float(i) / segments) * k2Pi), depth,
//				radius_top * Cos((float(i) / segments) * k2Pi));
		...
		m->m_indices.push_back(segments + i);
		m->m_indices.push_back(segments + nextIdx);

		m->m_indices.push_back(i);
		m->m_indices.push_back(segments + nextIdx);
		m->m_indices.push_back(nextIdx);

		m->m_indices.push_back(numVerts - 1);
		m->m_indices.push_back(i);
		m->m_indices.push_back(nextIdx);

		// back faces
//		m->m_indices.push_back(segments*2+i);
//		m->m_indices.push_back(segments*3 + nextIdx);
//		m->m_indices.push_back(segments*3 + i);
//
//		m->m_indices.push_back(segments*2+i);
...
Mesh* CreateDiscMesh(float radius, uint32_t segments) {
	...
	for (uint32_t i = 1; i <= segments; ++i) {
		...
		m->m_positions[nextVert] = Point3(
				radius * Sin((float(i) / segments) * k2Pi), 0.0f,
...
Mesh* CreateTetrahedron(float ground, float height) {
	...
	const Point3 vertices[4] = { Point3(-1.0f, ground, -dimValue), Point3(1.0f,
			ground, -dimValue), Point3(0.0f, ground + height, dimValue), Point3(
			0.0f, ground, dimValue) };
	...
	for (int i = 0; i <= 2 * slices + 1; ++i) {
		for (int j = 0; j <= segments; ++j) {
			...
			mesh->m_positions.push_back(
					Point3(x, y, z) * radius + Vec3(0.0f, yoffset, 0.0f));
			mesh->m_normals.push_back(Vec3(x, y, z));
